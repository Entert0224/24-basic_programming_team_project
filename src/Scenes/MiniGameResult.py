...
class MiniGameResult(Scene) :
    ...
    def Setup(cls) :
        ...
        cls.back_text = Sprite("assets/images/back_text.png",Vec2(SCREEN_WIDTH/2,SCREEN_HEIGHT/2 + 230), layer=20, color=pygame.Color(0,0,0,255))
        cls.back_button = cls.back_text.CreateButton()

        display_scores_text_ins = Text("",Vec2(SCREEN_WIDTH/2, SCREEN_HEIGHT/2 - 200), 35, fontpath="assets/fonts/H2HDRM.TTF")
        display_scores_text = ""
        ...
        description_text = Text("",Vec2(SCREEN_WIDTH/2, SCREEN_HEIGHT/2 + 180), size=30, fontpath="assets/fonts/H2HDRM.TTF") 

        winner_judge = [index + 1 for index, value in enumerate(Team.game_score_rank) if value == 1]
        winner_judge = map(str,winner_judge)
        winner_judge_text = ", ".join(winner_judge)
